...
void ImageView::draw(NVGcontext *ctx) {
    ...
    const float pixelDataMinDepth = 60.0f;

    if (scale() > pixelDataMinDepth && m_pixel_callback) {
        float font_size = scale() / 8.f;
        float alpha = std::min(1.f, (scale() - pixelDataMinDepth) / pixelDataMinDepth);
